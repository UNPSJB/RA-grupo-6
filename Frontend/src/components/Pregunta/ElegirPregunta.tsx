import { useState, useEffect } from "react";
import { Form, Button, Card, Badge} from "react-bootstrap";
import type { Pregunta} from "./PreguntaTypes";

type Props = {
  preguntasSeleccionadas: Pregunta[];
  setPreguntasSeleccionadas: (pregs: Pregunta[]) => void;
};

function ElegirPregunta({ preguntasSeleccionadas, setPreguntasSeleccionadas }: Props) {
  const [preguntasDisponibles, setPreguntasDisponibles] = useState<Pregunta[]>([]);
  const [preguntaSeleccionadaId, setPreguntaSeleccionadaId] = useState<number>();
  
  
  useEffect(() => {
    fetch("http://127.0.0.1:8000/preguntas/")
      .then(res => res.json())
      .then(data => setPreguntasDisponibles(data))
      .catch(err => console.error(err));
  }, []);

  const agregarPregunta = () => {

    const pregunta = preguntasDisponibles.find(p => p.id === preguntaSeleccionadaId);
    if (!pregunta) return;

    setPreguntasSeleccionadas([...preguntasSeleccionadas, pregunta]);
  };

  const preguntasParaSelect = preguntasDisponibles.filter(
    p => !preguntasSeleccionadas.some(s => s.id === p.id)
  );


return (
      <div>
          <div className="mb-3">
            <h5 className="mb-3 fw-semibold text-secondary" style={{ fontSize: "0.95rem" }}>
              Preguntas del Formulario
            </h5>
            {preguntasSeleccionadas.length > 0 && (
              <Badge bg="info" className="px-2 py-1" style={{ fontSize: "0.8rem" }}>
                {preguntasSeleccionadas.length} {preguntasSeleccionadas.length === 1 ? "pregunta" : "preguntas"}
              </Badge>
            )}
          </div>

          
          {preguntasSeleccionadas.length > 0 ? (
            <div className="mb-4">
              <div>
                {preguntasSeleccionadas.map((pregunta, index) => (
                  <Card 
                  key={pregunta.id} 
                  className="border" 
                  style={{ borderColor: "#e0e0e0"}}
                  >
                    <Card.Body className="p-3">
                      <div className="d-flex gap-2 align-items-center mb-2">
                        <Badge
                          bg="primary"
                          className="rounded-circle d-inline-flex justify-content-center align-items-center"
                          style={{ width: "24px", height: "24px", fontSize: "0.75rem" }}
                          >
                          {index + 1}
                        </Badge>
                        
                        <Badge
                          bg={pregunta.tipo === "Cerrada" ? "secondary" : "success"}
                          className="px-2 py-1"
                          style={{ fontSize: "0.7rem", fontWeight: "500" }}
                          >
                          {pregunta.tipo}
                        </Badge>
                      </div>

                      <p className="mb-2 fw-medium" style={{ color: "#2c3e50", fontSize: "0.9rem" }}>
                        {pregunta.texto}
                      </p>

                      {pregunta.tipo === "Cerrada" && pregunta.opciones.length > 0 && (
                        <div className="d-flex gap-2 flex-wrap mt-2">
                          {pregunta.opciones.map((op) => (
                            <span
                            key={op.id}
                            className="border rounded-pill px-2 py-1"
                            style={{
                              fontSize: "0.75rem",
                              backgroundColor: "#f8f9fa",
                              borderColor: "#dee2e6",
                              color: "#495057"
                            }}
                            >
                              {op.texto}
                            </span>
                          ))}
                        </div>
                      )}
                    </Card.Body>
                  </Card>
                ))}
              </div>
            </div>
          ) : (
            <div 
            className="text-center mb-3 rounded"
            style={{ 
              backgroundColor: "#f8f9fa", 
              border: "2px dashed #dee2e6",
              padding: "2rem 1rem"
            }}
            >
              <div style={{ fontSize: "2rem", opacity: 0.3 }}></div>
              <p className="text-muted mb-0 mt-2" style={{ fontSize: "0.85rem" }}>No hay preguntas agregadas aún</p>
              <small className="text-muted" style={{ fontSize: "0.75rem" }}>Selecciona una pregunta del menú de abajo</small>
            </div>
          )}

          <Form.Select
            value={preguntaSeleccionadaId}
            onChange={(e) => setPreguntaSeleccionadaId(Number(e.target.value))}
            className="border-2 mb-3"
            style={{ borderColor: "#dee2e6", padding: "0.75rem" }}
            >
            <option value="">Seleccione una pregunta...</option>
            {preguntasParaSelect.map((p) => (
              <option key={p.id} value={p.id}>
                {p.tipo} - {p.texto}
              </option>
            ))}
          </Form.Select>

          <div className="d-grid">
            <Button onClick={agregarPregunta}>
              + Agregar
            </Button>
          </div>
    </div>

  );
}

export default ElegirPregunta;


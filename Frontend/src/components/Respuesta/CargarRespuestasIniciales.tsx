...
export function cargarRespuestasIniciales(plantillaFormulario: any,instrumentoId: number): Promise<{respuestasSimples: RespuestaTemporal[];respuestasMultiples: { [grupoCuadroId: number]: InstanciaRespuestas[] };}> {
        ...
        const grupoPromises = Array.from(gruposCuadro).map((grupoCuadroId) => {
            ...
            return fetch(`http://127.0.0.1:8000/respuestas/fuente?pregunta_id=${preguntaConFuente.id}&instrumento_id=${instrumentoId}`)
                ...
                .then((prefillData) => {
                    ...
                    if (prefillData?.respuestas?.length > 0 && prefillData.multiple) {
